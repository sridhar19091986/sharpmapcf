...
#else
				    StreamReader sr = System.IO.File.OpenText(projfile);
                    string wkt = "";
				    wkt = sr.ReadToEnd();
                    //TODO
#endif
...
#else //LOG_a (x) can be converted to the base b by log_b(x)/log_b(a) 
		    double log2 = Math.Log10(this.GetFeatureCount())/Math.Log10(2);
            heur.maxdepth = (int)Math.Ceiling(Math.Log(log2));
#endif
			heur.minerror = 10;
			heur.tartricnt = 5;
			heur.mintricnt = 2;
