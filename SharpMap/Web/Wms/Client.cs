    ...
    public class Client
	{
...
#if !CFBuild //v2.0 no String.Contains, but equivalent
			if (!url.Contains("?"))
				strReq.Append("?");
#else
            if (url.IndexOf("?") != -1)
...
#else
            if (url.ToLower().IndexOf("service=wms") != -1)
                strReq.Append("SERVICE=WMS&");
            if (url.ToLower().IndexOf("request=getcapabilities") != -1)
                strReq.Append("REQUEST=GetCapabilities&");		
#endif
		...
		private WmsServerLayer ParseLayer(XmlNode xmlLayer)
		{
			...
			if (node != null)
			{
                ...
                try
                {
                    minx = double.Parse(node.Attributes["minx"].Value, System.Globalization.NumberStyles.Any, SharpMap.Map.numberFormat_EnUS);
                    miny = double.Parse(node.Attributes["miny"].Value, System.Globalization.NumberStyles.Any, SharpMap.Map.numberFormat_EnUS);
                    maxx = double.Parse(node.Attributes["maxx"].Value, System.Globalization.NumberStyles.Any, SharpMap.Map.numberFormat_EnUS);
                    maxy = double.Parse(node.Attributes["maxy"].Value, System.Globalization.NumberStyles.Any, SharpMap.Map.numberFormat_EnUS);
                }
                catch(Exception)
                {
                    throw new ArgumentException("Invalid LatLonBoundingBox on layer '" + layer.Name + "'");
