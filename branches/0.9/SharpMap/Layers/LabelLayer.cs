...
#else
            System.Drawing.Point position = map.WorldToImage(feature.GetBoundingBox().GetCentroid());
            position.X = (int)(position.X - size.Width * (short)style.HorizontalAlignment * 0.5f);
            position.Y = (int)(position.Y - size.Height * (short)style.VerticalAlignment * 0.5f);
#endif
